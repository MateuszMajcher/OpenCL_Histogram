...
int main(int argc, char **argv)
{
	...
	err = createAndBuildProgram("kernel.cl", context, &program);
	createKernel(program, "vecAdd", &kernel);
	...
	//cout << INT_MAX << endl;
	 ...
	 unsigned int rows = 8192;
	 unsigned int cols = 4096;
	 unsigned int size = rows*cols;

	const size_t bytes = size*sizeof(int);
	const size_t bytes_h = hist*sizeof(int);


	const size_t global_work_size[2] = { cols , rows };
	size_t local_work_size[2] = { 16 , 16 };

	int *h_a = new int[size];

	int *h_c = new int[hist];

	

	// Inicjalizacja wartosci
	loadValue(h_a, rows, cols);

	print2d(h_a, rows, cols);

	cl_mem d_a = clCreateBuffer(context, CL_MEM_READ_ONLY |
		CL_MEM_COPY_HOST_PTR, bytes, h_a, &err);

	cl_mem d_c = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
		bytes_h, NULL, &err);

	cl_mem d_rows = clCreateBuffer(context, CL_MEM_READ_ONLY |
		CL_MEM_COPY_HOST_PTR,
		sizeof(unsigned int), &rows, &err);
	cl_mem d_cols = clCreateBuffer(context, CL_MEM_READ_ONLY |
		CL_MEM_COPY_HOST_PTR,
		sizeof(unsigned int), &cols, &err);


	err = clSetKernelArg(kernel, 0, sizeof(cl_mem), &d_a);

	err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &d_c);
	err |= clSetKernelArg(kernel, 2, sizeof(unsigned int), &d_rows);
	err |= clSetKernelArg(kernel, 3, sizeof(unsigned int), &d_cols);
	...
	err = clEnqueueReadBuffer(command_queue, d_c, CL_TRUE,
		0, bytes_h, h_c, 0, NULL, NULL);
